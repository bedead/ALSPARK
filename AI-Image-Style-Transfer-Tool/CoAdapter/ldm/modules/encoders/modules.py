...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text):
        ...
        outputs = self.transformer(
            input_ids=tokens, output_hidden_states=self.layer != "last"
        )
...
class WebUIFrozenCLIPEmebedder(AbstractEncoder):
    def __init__(
        ...
        self.comma_token = [
            v for k, v in self.tokenizer.get_vocab().items() if k == ",</w>"
        ][0]
        ...
        for tokens, (text, weight) in zip(tokenized, parsed):
            ...
            while i < len(tokens):
                ...
                if token == self.comma_token:
                    ...
                    and max(len(remade_tokens), 1) % 75 == 0
                    and last_comma != -1
                    and len(remade_tokens) - last_comma <= self.comma_padding_backtrack
                ):
                    last_comma += 1
    ...
    def process_text(self, texts):
        ...
        for line in texts:
            if line in cache:
                ...
                remade_tokens, multipliers, current_token_count = self.tokenize_line(
                    line
                )
    ...
    def process_tokens(self, remade_batch_tokens, batch_multipliers):
        remade_batch_tokens = [
            [self.tokenizer.bos_token_id] + x[:75] + [self.tokenizer.eos_token_id]
        ...
        batch_multipliers = torch.asarray(batch_multipliers_of_same_length).to(
            self.device
        )
